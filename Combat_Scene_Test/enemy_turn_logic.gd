...
func return_active_enemy_parts(combat_direction: bool, enemy):
	if combat_direction == true:
		...
		rear_facing_enemy_parts_in_combat = enemy.rear_facing_body_parts.values()
		return rear_facing_enemy_parts_in_combat

func execute_enemy_turn(enemy_actor: Node2D, active_players_in_combat: Array):
	var target = active_players_in_combat.pick_random()
	...
	if target == enemy_actor:
		active_players_in_combat.erase(target)
		target = active_players_in_combat.pick_random()
