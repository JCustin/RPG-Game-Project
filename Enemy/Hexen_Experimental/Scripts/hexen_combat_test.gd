...
func highlight_bodypart(body_part: Sprite2D):
	if body_part == %Front_Body:
		%AnimationPlayer.stop()
		%AnimationPlayer.play("Front_Body_Targeted")
		
		
	if body_part == %Front_Right_Arm:
		%AnimationPlayer.stop()
		%AnimationPlayer.play("Front_Right_Arm_Targeted")
